...
class NoiseTest extends BaseTest {
  ...
  void testSimplexNoise() {
    ...
    for (var i = 0.0; i < values2D.length; ++i) {
      values2D[i.toInt()] = noise.noise2D(i, i);
      values3D[i.toInt()] = noise.noise3D(i, i, i);
    }

    // Ensure that querying those same coordinates repeats the cached value
    for (var i = 0.0; i < values2D.length; ++i) {
      expect(values2D[i.toInt()], equals(noise.noise2D(i, i)));
      expect(values3D[i.toInt()], equals(noise.noise3D(i, i, i)));
    }
  }
