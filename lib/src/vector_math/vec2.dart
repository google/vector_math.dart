...
class vec2 {
  ...
  /// Zero vector.
  vec2.zero() : storage = new Float32List(2);
  ...
  /// Negate.
  vec2 operator-() => new vec2(-storage[0], -storage[1]);

  /// Subtract two vectors.
  vec2 operator-(vec2 other) => new vec2(storage[0] - other.storage[0],
                                         storage[1] - other.storage[1]);
  ...
  vec2 operator/(double scale) {
    var o = 1.0 / scale;
    return new vec2(storage[0] * o, storage[1] * o);
  }
